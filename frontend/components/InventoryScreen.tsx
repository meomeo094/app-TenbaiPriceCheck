...
function profitClass(profit: number | null): string {
  ...
  if (profit > 0) return "font-bold text-emerald-400 text-lg";
  if (profit < 0) return "font-bold text-red-400 text-lg";
    ...
    <main className="min-h-screen bg-slate-900 flex flex-col">
      <header className="bg-slate-900/80 backdrop-blur-md sticky top-0 z-10 border-b border-slate-800 px-4 py-3 flex items-center gap-3 pt-[calc(0.75rem+env(safe-area-inset-top))]">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-white font-bold text-lg leading-tight">Kho &amp; lợi nhuận</h1>
          <p className="text-slate-400 text-xs">my_inventory — Inventory / Profit Checker</p>
        </div>
      </header>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-16 pt-5 max-w-2xl mx-auto w-full space-y-8">
        ...
        <section className="space-y-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="shrink-0 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:bg-slate-700 disabled:text-slate-500 px-5 py-3 text-base font-bold text-white shadow-lg shadow-emerald-900/30 touch-manipulation"
            >
              {profitLoading ? "Đang quét giá…" : "Check giá"}
            </button>
          </div>
        ...
        {profitRows && profitRows.length > 0 && (
          <section className="space-y-3">
            <h2 className="text-slate-200 text-sm font-semibold">Kết quả kaitori &amp; lợi nhuận</h2>
            {profitMeta && (
