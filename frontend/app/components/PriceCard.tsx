...
const SITE_COLORS: Record<string, string> = {
  ...
  "KaitoriShouten": "from-emerald-600/20 to-teal-600/10 border-emerald-500/30",
};

const BADGE_COLORS: Record<string, string> = {
  "GameKaitori": "bg-violet-500/20 text-violet-300",
  "1-chome": "bg-blue-500/20 text-blue-300",
  "KaitoriShouten": "bg-emerald-500/20 text-emerald-300",
};
...
export default function PriceCard({ result, rank }: PriceCardProps) {
  ...
  const colorClass = SITE_COLORS[result.site] || "from-slate-600/20 to-slate-700/10 border-slate-500/30";
  const badgeClass = BADGE_COLORS[result.site] || "bg-slate-500/20 text-slate-300";

  const isSuccess = result.status === "success" && result.price;
  const isNotFound = result.status === "not_found";

  return (
    <div className={`relative bg-gradient-to-br ${colorClass} border rounded-2xl p-5 transition-all duration-200 active:scale-95`}>
      {rank === 1 && isSuccess && (
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className={`inline-block text-xs font-medium px-2 py-0.5 rounded-full mb-1 ${badgeClass}`}>
              {result.site}
            </span>
            {isSuccess ? (
              <p className="text-2xl font-bold text-white">
                ¥{Number(result.price?.replace(/[^0-9]/g, "")).toLocaleString("ja-JP")}
              </p>
            ) : isNotFound ? (
